...
func newTestGCPNFSStorageDriverConfig(debugTraceFlags map[string]bool) *GCPNFSStorageDriverConfig {
	...
	config.CommonStorageDriverConfig.DebugTraceFlags = debugTraceFlags
	config.ProjectNumber = "1234567890987654321234567890"
	config.HostProjectNumber = "0987654321567890"
	config.APIKey = GCPPrivateKey{}
	config.APIRegion = "us-north"
	config.APIURL = "/endpoint"
	config.StorageDriverName = "gcp"
	config.StoragePrefix = sp("test_")
	config.APIAudienceURL = "/endpoint"
...
func TestGCPNFSStorageDriverConfig_String(t *testing.T) {
	config := newTestGCPNFSStorageDriverConfig(map[string]bool{"method": true})
	projectNumber := config.ProjectNumber
	hostProjectNumber := config.HostProjectNumber
	apiKey := config.APIKey
	configString := config.String()
	assert.NotContains(t, configString, projectNumber)
	assert.NotContains(t, configString, hostProjectNumber)
	assert.NotContains(t, configString, apiKey)
}

func TestGCPNFSStorageDriverConfig_GoString(t *testing.T) {
	config := newTestGCPNFSStorageDriverConfig(map[string]bool{"method": true})
	projectNumber := config.ProjectNumber
	hostProjectNumber := config.HostProjectNumber
	apiKey := config.APIKey
	configGoString := config.GoString()
	assert.NotContains(t, configGoString, projectNumber)
	assert.NotContains(t, configGoString, hostProjectNumber)
	assert.NotContains(t, configGoString, apiKey)
}

func TestGCPNFSStorageDriverConfig_InjectSecrets(t *testing.T) {
	tests := []struct {
		secretMap   map[string]string
		errorExists bool
	}{
...
func TestGCPNFSStorageDriverConfig_ExtractSecrets(t *testing.T) {
	privateKey := "key"
	privateKeyID := "id"
	config := GCPNFSStorageDriverConfig{
...
func TestGCPNFSStorageDriverConfig_GetAndHideSensitive(t *testing.T) {
	privateKey := "key"
	privateKeyID := "id"
	config := GCPNFSStorageDriverConfig{
		APIKey: GCPPrivateKey{
...
func TestGCPNFSStorageDriverConfig_CheckForCRDControllerForbiddenAttributes(t *testing.T) {
	privateKey := "key"
	privateKeyID := "id"
	config := GCPNFSStorageDriverConfig{
		APIKey: GCPPrivateKey{
	...
	forbiddenKeys := config.CheckForCRDControllerForbiddenAttributes()
	assert.Contains(t, forbiddenKeys, "Private_Key")
	assert.Contains(t, forbiddenKeys, "Private_Key_ID")
}

func TestGCPNFSStorageDriverConfig_SpecOnlyValidation(t *testing.T) {
	privateKey := "key"
	privateKeyID := "id"
	config := GCPNFSStorageDriverConfig{
		CommonStorageDriverConfig: &CommonStorageDriverConfig{
